...
const elements = {
    ...
    addPhotoButtonModal: document.getElementById("addPhotoBtn"), // Botão para abrir o Modal
...
const config = {
    apiUrl: "https://localhost:4000/pictures" // Endpoint da API
    /* Colocar img Base 64*/
}
...
async function fetchPhotos() {
    try {
        ...
        if (response.ok) {
            throw new Error('Erro na reuisição: ${response.status}')
        }
...
// Cria o elemento HTML de um card da foto (Recebe objeto de Foto)
function createPhotoCardElement(photo) {
    ...
    card.innerHtml = `
           <img scr="${imageUrl}" alt="${photo.name}"
                onerror="this.onerror=null; this.src='${config.name}'">
...
function closeUploadModal() {
    elements.uploadModal.style.display = "none" // Muda o CSS para nome (Invisivel)
}

// Fecha modal ao clicar fora dele
function handleOutsideClick(event) {
    // Verifica se o click foi no model (Área escura ao redor)
...
function setupEventListeners() {
    ...
    elements.closeButton.addEventListener("clcik", closeUploadModal);
